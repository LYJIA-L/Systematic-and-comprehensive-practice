...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        writeTransitRules(p4info_helper, sw=s2, tunnel_id=101, port_id=S2_TO_S1)
        ...
        writeTransitRules(p4info_helper, sw=s1, tunnel_id=200, port_id=S1_TO_S3)

        # h3 to h1
        writeTunnelRules(p4info_helper, ingress_sw=s3, egress_sw=s1, tunnel_id=201,
                         dst_eth_addr="08:00:00:00:01:11", dst_ip_addr="10.0.1.1")
        writeTransitRules(p4info_helper, sw=s3, tunnel_id=201, port_id=S3_TO_S1)

        #  h2 to h3
        writeTunnelRules(p4info_helper, ingress_sw=s2, egress_sw=s3, tunnel_id=300,
                         dst_eth_addr="08:00:00:00:03:33", dst_ip_addr="10.0.3.3")
        writeTransitRules(p4info_helper, sw=s2, tunnel_id=300, port_id=S2_TO_S3)

        # h3 to h2
        writeTunnelRules(p4info_helper, ingress_sw=s3, egress_sw=s2, tunnel_id=301,
                         dst_eth_addr="08:00:00:00:02:22", dst_ip_addr="10.0.2.2")
        writeTransitRules(p4info_helper, sw=s3, tunnel_id=301, port_id=S3_TO_S2)
